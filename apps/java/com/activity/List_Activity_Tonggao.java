 ...
 * 通知公告
 */
public class List_Activity_Tonggao extends Activity implements OnClickListener {
private int Page = 1;
private int Count = 1;
private int Num = 1;
...
private List<Tonggao> mDataList = new ArrayList<>();
HashMap<Integer, Boolean> isOpenMap = new HashMap<>();
protected void onCreate(Bundle savedInstanceState) {
super.onCreate(savedInstanceState);
requestWindowFeature(Window.FEATURE_NO_TITLE);
setContentView(R.layout.list_activity_tonggao);
initView();
setDataFromNet("",1);
}
...
Type type = new TypeToken<ArrayList<Tonggao>>() {
}.getType();
...
R.layout.item_list_activity_tonggao, null);
...
holder.function_1 = (Button) convertView.findViewById(R.id.function_1);
holder.function_2 = (Button) convertView.findViewById(R.id.function_2);
holder.ggbt = (TextView) convertView.findViewById(R.id.ggbt);
holder.ggnr = (TextView) convertView.findViewById(R.id.ggnr);
...
final Tonggao bean=mDataList.get(position);
holder.ggbt.setText("公告标题:"+bean.ggbt);
holder.ggnr.setText("公告内容:"+bean.ggnr);
holder.fbr.setText("发布人:"+bean.fbr);
holder.time.setText("发布时间:"+bean.time);
