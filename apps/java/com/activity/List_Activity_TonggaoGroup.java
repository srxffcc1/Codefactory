 ...
 * 通知公告分组
 */
public class List_Activity_TonggaoGroup extends Activity implements OnClickListener {
private int Page = 1;
private int Count = 1;
private int Num = 1;
...
private List<TonggaoGroup> mDataList = new ArrayList<>();
HashMap<Integer, Boolean> isOpenMap = new HashMap<>();
protected void onCreate(Bundle savedInstanceState) {
super.onCreate(savedInstanceState);
requestWindowFeature(Window.FEATURE_NO_TITLE);
setContentView(R.layout.list_activity_tonggaogroup);
initView();
setDataFromNet("",1);
}
...
Type type = new TypeToken<ArrayList<TonggaoGroup>>() {
}.getType();
...
R.layout.item_list_activity_tonggaogroup, null);
...
holder.fddbr = (TextView) convertView.findViewById(R.id.fddbr);
holder.isview = (TextView) convertView.findViewById(R.id.isview);
convertView.setTag(holder);
} else {
holder = (ViewHolder) convertView.getTag();
}
final TonggaoGroup bean=mDataList.get(position);
holder.uname.setText("用户名:"+bean.uname);
holder.fddbr.setText("法定代表人:"+bean.fddbr);
holder.isview.setText("是否查看:"+bean.isview);
