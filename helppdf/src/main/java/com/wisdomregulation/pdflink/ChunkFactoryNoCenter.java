...
/**
 * 本类 是 pdf打印的帮助类 不需要居中处理  因为此类需求少这个类就没有和ChunkFactory同步
 */
public class ChunkFactoryNoCenter {
    ...
    public static Chunk nounderLine(String text, float length, Font font, float firstLineIndent, float pagewidth) {
        Chunk targetChunk = null;
        try {
            targetChunk = new Chunk(StringFactoryBuild.build().stringFormatFixedLengthNoCenter(text, length, font, firstLineIndent, pagewidth), font)
                    .setSplitCharacter(new SplitCharacterFix());
        } catch (Exception e) {
            e.printStackTrace();
        }
