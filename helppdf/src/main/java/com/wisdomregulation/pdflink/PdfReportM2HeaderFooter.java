 ...
 *
 * @author 落雨
 * @version 394263788(QQ)   第几页共几页
 * @since JDK 1.5
 */
public class PdfReportM2HeaderFooter extends PdfPageEventHelper {
     ...
     *
     */
    public PdfReportM2HeaderFooter() {

    }
    String simdir;
    public PdfReportM2HeaderFooter(String simdir) {
        this.simdir=simdir;
    }
     ...
     */
    public PdfReportM2HeaderFooter(String yeMei, int presentFontSize, Rectangle pageSize) {
        this.header = yeMei;
    ...
    public void onEndPage(PdfWriter writer, Document document) {
        ...
        // 5.写入页脚1 原始数据如果页脚1和页脚2不对right进行修改的话都是“第几页共几页” 修改之后变成“第几页 共几页”
        //因为这里第几页写完已经再最右侧了 所以需要减去一个len理论上应该减去一个total不过减去len误差也不大再减去一个20空格
        ColumnText.showTextAligned(cb, Element.ALIGN_RIGHT, footer, document.right()-20-len, document.bottom(), 0);

        // 6.写入页脚2 因为是跟着页脚1后面的页脚1已经边缘了所有total减去一个自身就进入页边距范围了此处用len代替total自身误差也不大
        cb.addTemplate(total, document.right()-len, document.bottom() ); // 调节模版显示的位置
    }
